...
import com.nerox.old.connection.Easyproxy;
import com.nerox.old.connection.Easyreum;
import com.nerox.old.keepalives.UDPKeepAlive;
import com.nerox.old.misc.StatusInfo;
import com.nerox.old.misc.StatusServer;
import com.nerox.old.misc.TCPTimeouts;
...
public class TfprotocolSuper<ICallbackImplementation extends ISuperCallback>{
    ...
    public TfprotocolSuper(String ipServer, int portServer,
        ...
        clientSocket = new Client(ipServer, portServer);
    }
    /**
     * Constructor of tfprotocol
    ...
    public TfprotocolSuper(String proxy, String ipServer, int portServer,
        ...
        this.len = len;
        tcpTimeOut = TCPTimeouts.getInstance(protoHandler);
        clientSocket = new Client(proxy, ipServer, portServer);
    }
    ...
    /**
     * Get the client object which has the socket object in it and allows you to do some low level operation with it...
     * */
    ...
    public boolean isConnect() {
        return this.clientSocket.getSocket().isConnected();
    }
    /**
    ...
    public final Client connect() {
        this.easyreum = new Easyreum(this.clientSocket);
        StatusInfo status;
        int cont = 0;
        do {
            cont++;
            status = clientSocket.startConnection(tcpTimeOut.getConnectTimeout(), tcpTimeOut.getDnsResolutionTimeout());
    ...
    public final Client connect(UDPKeepAlive.TYPE keep_alive, int idle, int timeout, int max_tries) {
        ...
        int cont = 0;
        do {
            this.easyreum.getBuilder().setStatusInfo(this.clientSocket
